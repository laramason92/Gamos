...
RTGeneratorPhaseSpace::RTGeneratorPhaseSpace()
{
  ...
  theNEventsSkip = G4int(parmgr->GetNumericValue("RTGeneratorPhaseSpace:NEventsSkip", 0));
 ...
 if( RTVerb(infoVerb) ) G4cout << " NPART " << p_iaea_header->nParticles << " " << p_iaea_header->particle_number[0] << " " << p_iaea_header->particle_number[1]  << " " << p_iaea_header->particle_number[2] << " STAT "  << " " << p_iaea_header->minimumX << " " << p_iaea_header->maximumZ  << " " << p_iaea_header->minimumY << " " << p_iaea_header->maximumY << G4endl; 
#endif
  GmNumberOfEvent::SetNumberOfEventFactor( G4double(theNOrigEvents) / theNEventsInFile );  // temporary: it does not take into account the weights of the particles 
#ifndef GAMOS_NO_VERBOSE
  if( RTVerb(infoVerb) )  G4cout << "RTGeneratorPhaseSpace::GeneratePrimaries   SetNumberOfEventFactor " << G4double(theNOrigEvents) / theNEventsInFile  << " " << G4double(theNOrigEvents) << " " << theNEventsInFile << G4endl;
#endif
  if( theNEventsInFile <= theNEventsSkip ){
    ...
    G4Exception("RTGeneratorPhaseSpace:",
		...
		JustWarning,
		G4String("NEventsInFile= " + GmGenUtils::itoa(theNEventsInFile) + " <= NEventsSkip= " + GmGenUtils::itoa(theNEventsSkip)).c_str());
  ...
  if( bHistos ) theHistos = new RTPhaseSpaceHistos(p_iaea_header->record_constant[2]*cm ); 
...
void RTGeneratorPhaseSpace::GeneratePrimaries(G4Event* evt)
{
  if( bMaxNReuseAutomatic && evt->GetEventID() == 0 ) {
    ...
    theMaxNReuse = G4int( G4RunManager::GetRunManager()->GetCurrentRun()->GetNumberOfEventToBeProcessed() / theNEventsInFile ) + 1;
    theMaxNRecycle = 1; 
#ifndef GAMOS_NO_VERBOSE
  ...
  if( bHistos ) theHistos->FillHistos( p_iaea_read, p_iaea_read->z*cm, (G4Step*)0 );
