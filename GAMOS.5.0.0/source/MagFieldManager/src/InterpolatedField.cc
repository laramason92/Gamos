...
{ 
  ...
  for (int i=0;i<n1;i++){
	...
	for (int j=0;j<n2;j++){
		...
		for (int k=0;k<n3;k++){
			...
			B1_grid3D[i][j].push_back(b1*tesla);
			B2_grid3D[i][j].push_back(b2*tesla);
			B3_grid3D[i][j].push_back(b3*tesla);
  ...
  if (IsCartesian){
	...
	G4cout<<"Cylindrical "<<'\t'<<drho/m<<'\t'<<dphi/degree<<'\t'<<dz/m<<std::endl;
  }	
}
...
{ G4int i,j,k;
  G4double x,y,z,rho,phi,cphi,sphi;
