...
{G4double y2= y*y;
 ...
 G4double rmm=std::sqrt(rhomm2+y2);
 G4double rpm=std::sqrt(rhopm2+y2);
 G4double rmp=std::sqrt(rhomp2+y2);
 G4double rpp=std::sqrt(rhopp2+y2);
...
{ 
 ...
 beginning_of_integration = false;
 G4double rhom2=x2+ym2;
 if (rhom2<mm2) rhom2 = mm2; 
 G4double rhop2=x2+yp2;
