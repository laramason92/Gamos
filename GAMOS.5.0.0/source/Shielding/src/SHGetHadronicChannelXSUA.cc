...
{ 
  ...
  if( bFirstEventInRun ) { // only once per world
	...
	nAtomsPermm3 += theCurrentAtomNumDensityVector[iele];
      ...
      theCrossSections[hadProc] = cross/nAtomsPermm3/barn;
      G4cout << "@@SHGetHadronicChannelXSUA " << theCurrentMaterial->GetName() << " " 
	     ...
	     << " TOTAL_XS(micro)= " << cross/nAtomsPermm3/barn << " barn" 
	     << G4endl;
	...
	hisXSmicro->Fill(thePrimaryKineticEnergy,cross);
	GmHisto1* hisXSmacro = theAnaMgr->GetHisto1( theHistoNumber+materialID*100000+iproc*10000+2);
	hisXSmacro->Fill(thePrimaryKineticEnergy,cross/nAtomsPermm3/barn);
      }	
    }
  }
...
void SHGetHadronicChannelXSUA::EndOfRunAction(const G4Run*)
{
  ...
  for( itemh = theProcessSecoParticles.begin(); itemh != theProcessSecoParticles.end(); itemh++ ) {
    ...
    std::map<G4String, G4int >::const_iterator iteci;
	...
	hisNP->Fill( thePrimaryKineticEnergy, G4double((*iteci).second)/theNEventsInRun);
	hisXSmic->Fill( thePrimaryKineticEnergy, (*iteci).second*(*iteXS).second/ theNEventsInRun );
      }
	...
	for( itepy1 = partYield->begin(); itepy1 != partYield->end(); itepy1++ ) {
	  ...
	  if( bHistos ) {
	    ...
	    hisY->Fill( thePrimaryKineticEnergy, G4double((*itepy1).second)/theNEventsInRun);
	    hisXSmic->Fill( thePrimaryKineticEnergy, (*itepy1).second*(*iteXS).second/ theNEventsInRun );
	  }
