...
void GmNoUseG4RadDecayTimeUA::BeginOfRunAction( const G4Run* )
{
  ...
  theMaxPrimaryTime = 0.;
  theMaxTracksTime = GmParameterMgr::GetInstance()->GetNumericValue(theName+":MaximumTracksTime",1.e-8*second);
}
...
#ifndef GAMOS_NO_VERBOSE
  if( SHVerb( debugVerb ) ) G4cout << " GmNoUseG4RadDecayTimeUA::ClassifyNewTrack  track postponed " << aTrack->GetGlobalTime() << " > " << theMaxPrimaryTime + theMaxTracksTime 
