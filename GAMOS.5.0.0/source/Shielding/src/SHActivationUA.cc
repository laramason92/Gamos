...
void SHActivationUA::InitialiseRun()
{
  ...
  if( maxPeriod < maxTime ) {
    thePeriods[ maxTime - maxPeriod ] = 0; 
#ifndef GAMOS_NO_VERBOSE
    if( SHVerb(debugVerb) ) G4cout << theName << "::BeginOfRunAction defining  last period " << maxTime - maxPeriod << "  0 " << G4endl;
#endif
  }

  theRadDecayChainSet->SetPeriods( thePeriods );

  G4double minHalfLife = GmParameterMgr::GetInstance()->GetNumericValue(theName+":MinHalfLife",1.*second);
...
#ifndef GAMOS_NO_VERBOSE
  if( SHVerb(debugVerb) )  G4cout << "SHActivationUA::EndOfRunAction NEventsInjob " << nev << " JobTime " << theMaximumTime-theMinimumTime << " NEventsPerNanoSecond " << nev / (theMaximumTime-theMinimumTime) << " time min " << theMinimumTime << " max " << theMaximumTime << G4endl;
#endif
