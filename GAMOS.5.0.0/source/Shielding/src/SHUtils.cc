...
//------------------------------------------------------------------
void SHUtils::BuildChannelList(const G4ParticleDefinition* part, G4String UAName)
{
  ...
  G4int nProtonsProjectile = -part->GetAtomicNumber();
  G4int nNeutronsProjectile = -(part->GetAtomicMass() - nProtonsProjectile);
  for(;;){
