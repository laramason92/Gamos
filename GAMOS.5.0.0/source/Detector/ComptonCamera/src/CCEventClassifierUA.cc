...
bDumpMultiples = G4bool(GmParameterMgr::GetInstance()->GetNumericValue("CC:EvtClass:DumpMultiples",0));
  ...
  theGammaEnergy =GmParameterMgr::GetInstance()->GetNumericValue("CC:EvtClass:GammaEnergy",141.*keV);
  ...
  theComptonRecHitsDistScat  = GmParameterMgr::GetInstance()->GetNumericValue("CC:EvtClass:ComptonRecHitDistScat",0.*mm);
  theComptonRecHitsDistAbs  = GmParameterMgr::GetInstance()->GetNumericValue("CC:EvtClass:ComptonRecHitDistAbs",0.*mm);
...
void CCEventClassifierUA::BookHistos()
{
  ...
  std::string hnam;
  std::string hgnam = "CCEvtClass: ";
  hnam = hgnam + std::string("CC classification");
  theAnaMgr->CreateHisto1D(hnam,1200,0,1200.,320000+1);
  hnam = hgnam + std::string("Sum energy all coincident events (keV)");
  theAnaMgr->CreateHisto1D(hnam,1000,0,1000.,320000+2);
  hnam = hgnam + std::string("Scatter detector energy all coincident events (keV)");
  theAnaMgr->CreateHisto1D(hnam,1000,0,1000.,320000+3);
  hnam = hgnam + std::string("Absorber detector energy all coincident events (keV)");
  theAnaMgr->CreateHisto1D(hnam,1000,0,1000.,320000+4);
  hnam = hgnam + std::string("Scatter detector energy fully absorbed coincident events (keV)");
  theAnaMgr->CreateHisto1D(hnam, 1000,0,1000.,320000+5);
  hnam = hgnam + std::string("Absorber detector energy fully absorbed coincident events (keV)");
  ...
  hnam = hgnam + std::string("Sum energy all fully absorbed events (keV)");
  theAnaMgr->CreateHisto1D(hnam,1000,0,1000.,320000+9);
  hnam = hgnam + std::string("Sum energy all fully absorbed single_single events (keV)");
  theAnaMgr->CreateHisto1D(hnam,1000,0,1000.,320000+10);
  hnam = hgnam + std::string("Scatter detector energy fully absorbed single_single events (keV)");
  theAnaMgr->CreateHisto1D(hnam, 1000,0,1000.,320000+11);
  hnam = hgnam + std::string("Absorber detector energy fully absorbed single_single events (keV)");
  theAnaMgr->CreateHisto1D(hnam,1000,0,1000.,320000+12);
 hnam = hgnam + std::string("Absorber detector energy single_single events (keV)");
  theAnaMgr->CreateHisto1D(hnam,1000,0,1000.,320000+13);
 hnam = hgnam + std::string("Scatterer detector energy single_single events (keV)");
  theAnaMgr->CreateHisto1D(hnam,1000,0,1000.,320000+14);
  hnam = hgnam + std::string("Sum energy all single_single events (keV)");
...
//----------------------------------------------------------------------
G4int CCEventClassifierUA::Classify( gamosRecHitList& rhitList, const G4Event* evt  )
{
  G4int clCC = ClassifyCC( rhitList, evt );
  if( clCC != 0 ) {
...
//----------------------------------------------------------------------
void CCEventClassifierUA::DeleteHitsInRecHits()
{
  // DAQ is triggered when there is 2 addback recHits, then recHit hits are read 
  // and they should not be in any other event classification: delete them
  if( theEventClass == 0 ) return; 
  DetRecHitSetList::iterator ites;
  std::set<GmRecHit*>::iterator ite;
  for( ites = theFullRecHits.begin(); ites != theFullRecHits.end(); ites++ ){
...
G4int CCEventClassifierUA::ClassifyCC( gamosRecHitList& rhitList, const G4Event* evt )
{
 ...
 // If there is one unique incident gamma in two sensitive detector types, process the event
  if( theFullRecHits.size()==2 ) {
   ...
   { 
    ...
    theAnaMgr->GetHisto1(320000+2)->Fill(sumEnergy/keV);
    theAnaMgr->GetHisto1(320000+3)->Fill(scatEnergy/keV);
    theAnaMgr->GetHisto1(320000+4)->Fill(absEnergy/keV);
    ...
    if (sumEnergy < thePhotopeakEPrecMin || sumEnergy > thePhotopeakEPrecMax)
     { 
       ...
       if ((scatHits==1) && (absHits==1)) {
         theAnaMgr->GetHisto1(320000+14)->Fill(scatEnergy/keV);
         theAnaMgr->GetHisto1(320000+13)->Fill(absEnergy/keV);
         theAnaMgr->GetHisto1(320000+15)->Fill(sumEnergy/keV);
       ...
       theAnaMgr->GetHisto1(320000+5)->Fill(scatEnergy/keV);
       theAnaMgr->GetHisto1(320000+6)->Fill(absEnergy/keV);
       theAnaMgr->GetHisto1(320000+9)->Fill(sumEnergy/keV);

       //Write event to file for single single coincident
       if ((scatHits==1) && (absHits==1)) {
        theEventClass +=1;
        theAnaMgr->GetHisto1(320000+10)->Fill(sumEnergy/keV);
        theAnaMgr->GetHisto1(320000+11)->Fill(scatEnergy/keV);
        theAnaMgr->GetHisto1(320000+12)->Fill(absEnergy/keV);
        theAnaMgr->GetHisto1(320000+14)->Fill(scatEnergy/keV);
        theAnaMgr->GetHisto1(320000+13)->Fill(absEnergy/keV);
        theAnaMgr->GetHisto1(320000+15)->Fill(sumEnergy/keV);
        ...
        if( bDump && theEventClass != 0) {
         if (bDumpSingles==1) { 
          CCIOMgr::GetInstance()->WriteEvent(theFullRecHits[0].GetPosition(),theFullRecHits[0].GetEnergy()/keV,theFullRecHits[1].GetPosition(), theFullRecHits[1].GetEnergy()/keV);
         }
        }
        singles++;
       } else {
...
if( fabs(seedRecHit->GetTime()-newRecHit->GetTime()) > theCoincidenceTime ){
  ...
  #ifndef GAMOS_NO_VERBOSE
   if( CCVerb(debugVerb) ) G4cout << "CCEventClassifierUA::GetFullComptonRecHits: firstrecHitSet initial pos  LOOP4   " << firstrecHitSet.GetPosition()<< "   firstrecHitSize  "<< firstrecHitSet.size()<<   " with a distenergcomptlistsize " <<distEnerComptRecHitList.size()<<"  and an energy of "<<firstrecHitSet.GetEnergy()/keV<<G4endl;
  #endif
...
//----------------------------------------------------------------------
void CCEventClassifierUA::PrintCCClassificationTable( G4int nevt )
{
  GmAnalysisMgr*  theAnaMgr = GmAnalysisMgr::GetInstance("compcam") ;
  ...
  G4cout << "       ALL                          :  " << b_00+b_01+b_02+b_03+b_10+b_11+b_12+b_13<< "   " << (b_00+b_01+b_02+b_03+b_10+b_11+b_12+b_13)/nevt*100 << " % " << G4endl; 
  G4cout << "(   0) Not coincident events:  " <<b_00<<"  " <<(b_00)/nevt*100 << " % " <<G4endl; 
  G4cout << "(   1) Coincident events   :  " << b_01+b_02+b_03 << "   " << (b_01+b_02+b_03)/nevt*100 << " % " << G4endl; 
  G4cout << "(   2) Fully absorbed coincident event :  " << b_02+b_03 << "   " << (b_02+b_03)/nevt*100 << " % " << G4endl; 
  G4cout << "(   3) Compton imaging Single Single events    :  " << b_03 << "   " << b_03/nevt*100 << " % " << G4endl; 
  G4cout << "   More than 2 reconstructed hits found :  " <<b_10+b_11+b_12+b_13 <<"  " <<(b_10+b_11+b_12+b_13 )/nevt*100 << " % " << G4endl;
  G4cout <<" "<<G4endl;

  G4cout << " --------------- Random Coincidences -------------" << G4endl;
  G4cout << "       ALL                          :  " << b_100+b_101+b_102+b_103+b_110+b_111+b_112+b_113 << "   " << (b_100+b_101+b_102+b_103+b_110+b_111+b_112+b_113)/nevt*100 << " % " << G4endl; 
  G4cout << "( 100) Not coincident events:  " <<b_100<<"  " <<(b_100)/nevt*100 << " % " <<G4endl; 
  G4cout << "( 101) Coincidence events    :  " << b_101+b_102+b_103 << "   " << (b_101+b_102+b_103)/nevt*100 << " % " << G4endl;
  G4cout << "( 102) Fully absorbed coincident event :  " << b_102+b_103 << "   " << (b_102+b_103)/nevt*100 << " % " << G4endl; 
  G4cout << "( 103) Compton imaging Single Single events    :  " << b_103 << "   " << b_103/nevt*100 << " % " << G4endl; 
  G4cout << "   More than 2 reconstructed hits found :  " <<b_110+b_111+b_112+b_113 <<"  " <<(b_110+b_111+b_112+b_113 )/nevt*100 << " % " << G4endl;
  G4cout <<" "<<G4endl;

  G4cout << " --------------- Scattered -------------" << G4endl;
  G4cout << "       ALL                          :  " <<  b_1000+b_1001+b_1002+b_1003+b_1010+b_1011+b_1012+b_1013 <<" " << (b_1000+b_1001+b_1002+b_1003+b_1010+b_1011+b_1012+b_1013)/nevt*100 << " % " << G4endl; 
  G4cout << "(1000)  Not coincident events  :  " << b_1000<<"  " <<(b_1000)/nevt*100 << " % " <<G4endl; 
  G4cout << "(1001) Coincidence events    :  " << b_1001+b_1002+b_1003 << "   " << (b_1001+b_1002+b_1003)/nevt*100 << " % " << G4endl;
  G4cout << "(1002) Fully absorbed coincident event :  " << b_1002+b_1003 << "   " << (b_1002+b_1003)/nevt*100 << " % " << G4endl; 
  G4cout << "(1003) Compton imaging Single Single events    :  " << b_1003 << "   " << b_1003/nevt*100 << " % " << G4endl; 
  G4cout << "   More than 2 reconstructed hits found :  "<<b_1010+b_1011+b_1012+b_1013<<"  " <<(b_1010+b_1011+b_1012+b_1013)/nevt*100 << " % " <<G4endl;
  G4cout <<" "<<G4endl;
 
  G4cout << " --------------- Scattered and Random Coincidences -------------" << G4endl;
  G4cout << "       ALL                          :  " << b_1100+b_1101+b_1102+b_1103+b_1110+b_1111+b_1112+b_1113 << "   " << ( b_1100+b_1101+b_1102+b_1103+b_1110+b_1111+b_1112+b_1113 )/nevt*100 << " % " << G4endl; 
  G4cout << "(1100)  Not coincident events  :  " << b_1100<<"  " <<(b_1100)/nevt*100 << " % " <<G4endl;
  G4cout << "(1101) Coincidence events    :  " << b_1101+b_1102+b_1103 << "   " << (b_1101+b_1102+b_1103)/nevt*100 << " % " << G4endl;
  G4cout << "(1102) Fully absorbed coincident event :  " << b_1102+b_1103 << "   " << (b_1102+b_1103)/nevt*100 << " % " << G4endl; 
  G4cout << "(1103) Compton imaging Single Single events    :  " << b_1103 << "   " << b_1103/nevt*100 << " % " << G4endl; 
  G4cout << "   More than 2 reconstructed hits found :  " <<b_1110+b_1111+b_1112+b_1113 <<"  " <<(b_1110+b_1111+b_1112+b_1113 )/nevt*100 << " % " << G4endl;
}  
