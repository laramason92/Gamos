...
PETEventClassifierUA::PETEventClassifierUA()
{
  ...
  the511EPrecMin = GmParameterMgr::GetInstance()->GetNumericValue("PET:EvtClass:511EPrecMin",electron_mass_c2*(1-EPrec511));
  the511EPrecMax = GmParameterMgr::GetInstance()->GetNumericValue("PET:EvtClass:511EPrecMax",electron_mass_c2*(1+EPrec511));
  ...
  thePETLineDistToVtx  = GmParameterMgr::GetInstance()->GetNumericValue("PET:EvtClass:LineDistToVtx",10.*mm);

  theComptonRecHitsDist  = GmParameterMgr::GetInstance()->GetNumericValue("PET:EvtClass:ComptonRecHitDist",0.*mm);
...
void PETEventClassifierUA::BookHistos()
{
  ...
  hnam = hgnam + std::string("PET dist line - vertex PHI(mm)");
  theAnaMgr->CreateHisto1D(hnam,100,0,25.,320000+7);
  hnam = hgnam + std::string("PET RecHit energy (keV)");
  theAnaMgr->CreateHisto1D(hnam,600,0,600.,320000+8);
  hnam = hgnam + std::string("Extra PET RecHit energy (keV)");
...
//----------------------------------------------------------------------
G4int PETEventClassifierUA::Classify( gamosRecHitList& rhitList, const G4Event* evt  )
{
  G4int clpet = ClassifyPET( rhitList, evt );
...
//----------------------------------------------------------------------
void PETEventClassifierUA::DeleteHitsInRecHits()
{
  //----- DAQ is triggered when there are 2 511 recHits, then recHit hits are read and they should not be in any other event classification: delete them
  if( theEventClass == 0 ) return;
 
  DetRecHitSetList::iterator ites;
  std::set<GmRecHit*>::iterator ite;
  for( ites = the511RecHits.begin(); ites != the511RecHits.end(); ites++ ){
...
//----------------------------------------------------------------------
G4int PETEventClassifierUA::ClassifyPET( gamosRecHitList& rhitList, const G4Event* evt )
{
  the511RecHits.clear();
  ...
  if( the511RecHits.size() == 2 ) {
    ...
    if( the511RecHits[1].GetPosition() != the511RecHits[0].GetPosition() ) {
      ...
      theEventClass += G4int( linedist > thePETLineDistToVtx );
...
void PETEventClassifierUA::FillDCAHistos( G4double linedist )
{
  ...
  for( unsigned int ii = 1; ii < 5; ii++ ){  
    if( ((theEventClass-1)/G4int(pow(float(10),int(ii-1)))%10) == 1 ) {
      //      G4cout << " FillALL " << 341000+ii << " " << theAnaMgr->GetHisto1(341000+ii)->GetName() << " " << theEventClass << " " <<  ((theEventClass-1)/G4int(pow(10,ii-1))%10)  << G4endl;
...
//----------------------------------------------------------------------
void PETEventClassifierUA::PrintPETClassificationTable( G4int nevt )
{
  GmAnalysisMgr*  theAnaMgr = GmAnalysisMgr::GetInstance("pet") ;
  ...
  G4cout << " ---------------------------------------------" << G4endl;
  G4cout << " Events PET               :  " << b_PET << "   " << ( b_PET)/nevt*100 << " % " << G4endl;
  //  G4cout << " Events PET               :  " << nevt-b_00 << "   " << (nevt-b_00)/nevt*100 << " % " << G4endl;

  G4cout << " --------------- Good PET -------------" << G4endl;
  G4cout << "       ALL                          :  " << b_01+b_02+b_11+b_12 << "   " << (b_01+b_02+b_11+b_12)/nevt*100 << " % " << G4endl; 
  ...
  G4cout << "(  11) PET 3->2 recHit close to vtx :  " << b_11 << "   " << b_11/(G4double(nevt))*100 << " % " << G4endl;
  G4cout << "(  12) PET 3->2 recHit far from vtx :  " << b_12 << "   " << b_12/(G4double(nevt))*100 << " % " << G4endl;
  ...
  G4cout << "       ALL                          :  " << b_101+b_102+b_111+b_112 << "   " << (b_101+b_102+b_111+b_112)/nevt*100 << " % " << G4endl; 
  ...
  G4cout << "( 111) PET 3->2 recHit close to vtx :  " << b_111 << "   " << b_111/(G4double(nevt))*100 << " % " << G4endl;
  G4cout << "( 112) PET 3->2 recHit far from vtx :  " << b_112 << "   " << b_112/(G4double(nevt))*100 << " % " << G4endl;
  ...
  G4cout << "       ALL                          :  " << b_1001+b_1002+b_1011+b_1012 << "   " << (b_1001+b_1002+b_1011+b_1012)/nevt*100 << " % " << G4endl; 
  ...
  G4cout << "       ALL                          :  " << b_1101+b_1102+b_1111+b_1112 << "   " << (b_1101+b_1102+b_1111+b_1112)/nevt*100 << " % " << G4endl; 
