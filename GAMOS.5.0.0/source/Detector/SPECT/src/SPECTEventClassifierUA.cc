...
#include "Detector/Base/include/DetV1stHitAlgorithm.hh"
#include "Detector/Base/include/DetComptonStudyHistosUA.hh"
#include "Detector/Base/include/DetCountScatteringUA.hh"
...
SPECTEventClassifierUA::SPECTEventClassifierUA()
{
  ...
  theSPECTLineDistToVtx  = paramMgr->GetNumericValue("SPECT:EvtClass:LineDistToVtx",10.*mm);
  
  theComptonRecHitsDist  = paramMgr->GetNumericValue("SPECT:EvtClass:ComptonRecHitDist",0.*mm);
...
void SPECTEventClassifierUA::GetCollimatorCentres()
{
  ...
  */
  ...
  for( unsigned int ii = 0; ii < touchs.size(); ii++ ) {
...
void SPECTEventClassifierUA::BookHistos()
{
  ...
  std::string hgnam = "SPECTEvtClass: ";
  ...
  hnam = hgnam + std::string("dist line - vertex RPHI (mm)");
  ...
  hnam = hgnam + " ALL far " + ": dist line - vertex (mm)";
  ...
  hnam = hgnam + " ALL 3->2 " + ": dist line - vertex (mm)";
  ...
  hnam = hgnam + " ALL random coincidences " + ": dist line - vertex (mm)";
  ...
  hnam = hgnam + " ALL scattered " + ": dist line - vertex (mm)";
...
//----------------------------------------------------------------------
G4int SPECTEventClassifierUA::Classify( gamosRecHitList& rhitList, const G4Event* evt  )
{
  G4int clpet = ClassifySPECT( rhitList, evt );

  if( clpet != 0 ) {
    DeleteHitsInRecHits(); // recHit will be deleted automatically
  }
...
//----------------------------------------------------------------------
G4int SPECTEventClassifierUA::ClassifySPECT( gamosRecHitList& rhitList, const G4Event* evt )
{
  ...
  if( theGoodRecHits.size() == 1 ) {
   ...
   theAnaMgr->GetHisto1(320000+5)->Fill( theGoodRecHits[0].GetEnergy()/keV );
    ...
    linedist = 0;
    theEventClass += G4int( linedist > theSPECTLineDistToVtx );
...
//----------------------------------------------------------------------
void SPECTEventClassifierUA::DeleteHitsInRecHits()
{
  //----- DAQ is triggered when there are 2 good recHits, then recHit hits are read and they should not be in any other event classification: delete them
  if( theEventClass == 0 ) return;
 
  DetRecHitSetList::iterator ites;
  std::set<GmRecHit*>::iterator ite;
  for( ites = theGoodRecHits.begin(); ites != theGoodRecHits.end(); ites++ ){
    std::set<GmRecHit*> rhitset = *ites;
    for( ite = rhitset.begin(); ite != rhitset.end(); ite++ ){
      theHitsEventMgr->DeleteHits( *ite );
    }
  }
}
...
void SPECTEventClassifierUA::FillDCAHistos( G4double linedist )
{
  ...
  theAnaMgr->GetHisto1(331000+theEventClass-1)->Fill( theGoodRecHits[0].GetEnergy()/keV );
  for( unsigned int ii = 1; ii < 5; ii++ ){  
    if( ((theEventClass-1)/G4int(pow(float(10),int(ii-1)))%10) == 1 ) {
      //      G4cout << " FillALL " << 341000+ii << " " << theAnaMgr->GetHisto1(341000+ii)->GetName() << G4endl;
      theAnaMgr->GetHisto1(341000+ii)->Fill( linedist );
      theAnaMgr->GetHisto1(351000+ii)->Fill( theGoodRecHits[0].GetEnergy()/keV );
    }
  }
...
//----------------------------------------------------------------------
void SPECTEventClassifierUA::PrintSPECTClassificationTable( G4int nevt )
{
  ...
  G4double b_01 = theEventClassifierMap[1]; // SPECT event, 1 recHit of theGammaEnergy keV, LOR < 4mm from source
  G4double b_02 = theEventClassifierMap[2]; // SPECT event, 1 recHit of theGammaEnergy keV, LOR > 4mm from source
  
  G4double b_11 = theEventClassifierMap[11]; // SPECT event, 2->1 recHits of theGammaEnergy keV, LOR < 4mm from source
  G4double b_12 = theEventClassifierMap[12]; // SPECT event, 2->1 recHits of theGammaEnergy keV, LOR > 4mm from source
  

  // Print table for SPECT
  G4double b_SPECT = theEventClassifierMap[99999];
  G4cout << " ---------------------------------------------" << G4endl;
  G4cout << " Events SPECT                          : " << b_SPECT << "   " << ( b_SPECT)/nevt*100 << " % " << G4endl;
  //  G4cout << " Events SPECT               :  " << nevt-b_00 << "   " << (nevt-b_00)/nevt*100 << " % " << G4endl;

  G4cout << " --------------- Good SPECT -------------" << G4endl;
  G4cout << "       ALL GOOD                        : " << nGOOD << "   " << nGOOD/nevt*100 << " % " << G4endl; 
  G4cout << "(    1) SPECT 2 recHits close to vtx   : " << b_01 << "   " << b_01/nevt*100 << " % " << G4endl; 
  G4cout << "(    2) SPECT 1 recHit far from vtx    : " << b_02 << "   " << b_02/nevt*100 << " % " << G4endl;
  G4cout << "(   11) SPECT 2->1 recHit close to vtx : " << b_11 << "   " << b_11/(G4double(nevt))*100 << " % " << G4endl;
  G4cout << "(   12) SPECT 2->1 recHit far from vtx : " << b_12 << "   " << b_12/(G4double(nevt))*100 << " % " << G4endl;
  
  // recHits with hits from different original gammas
  G4double b_101 = theEventClassifierMap[101]; // SPECT event, 1 recHit of theGammaEnergy keV LOR < 4mm from source
  G4double b_102 = theEventClassifierMap[102]; // SPECT event, 1 recHit of theGammaEnergy keV, LOR > 4mm from source
  
  G4double b_111 = theEventClassifierMap[111]; // SPECT event, 2->1 recHits of theGammaEnergy keV, LOR < 4mm from source
  G4double b_112 = theEventClassifierMap[112]; // SPECT event, 2->1 recHits of theGammaEnergy keV, LOR > 4mm from source
  
  G4cout << " --------------- Random Coincidences -------------" << G4endl;
  G4cout << "       ALL RANDOM COINCIDENCES         : " << nCOINC << "   " << nCOINC/nevt*100 << " % " << G4endl; 
  ...
  G4cout << "(  111) SPECT 2->1 recHit close to vtx : " << b_111 << "   " << b_111/(G4double(nevt))*100 << " % " << G4endl;
  G4cout << "(  112) SPECT 2->1 recHit far from vtx : " << b_112 << "   " << b_112/(G4double(nevt))*100 << " % " << G4endl;

  // recHits with hits from different original gammas
  G4double b_1001 = theEventClassifierMap[1001]; // SPECT event, 1 recHit of theGammaEnergy keV, LOR < 4mm from source
  G4double b_1002 = theEventClassifierMap[1002]; // SPECT event, 1 recHit of theGammaEnergy keV, LOR > 4mm from source
  
  G4double b_1011 = theEventClassifierMap[1011]; // SPECT event, 2->1 recHits of theGammaEnergy keV, LOR < 4mm from source
  G4double b_1012 = theEventClassifierMap[1012]; // SPECT event, 2->1 recHits of theGammaEnergy keV, LOR > 4mm from source
  
  G4cout << " --------------- Scattered -------------" << G4endl;
  G4cout << "       ALL SCATTERED                   : " << nSCATT << "   " << nSCATT/nevt*100 << " % " << G4endl; 
  ...
  G4cout << "( 1011) SPECT 2->1 recHit close to vtx : " << b_1011 << "   " << b_1011/(G4double(nevt))*100 << " % " << G4endl;
  G4cout << "( 1012) SPECT 2->1 recHit far from vtx : " << b_1012 << "   " << b_1012/(G4double(nevt))*100 << " % " << G4endl;
  
  // recHits with hits from different original gammas
  G4double b_1101 = theEventClassifierMap[1101]; // SPECT event, 1 recHit of theGammaEnergy keV, LOR < 4mm from source
  G4double b_1102 = theEventClassifierMap[1102]; // SPECT event, 1 recHit of theGammaEnergy keV, LOR > 4mm from source
  
  G4double b_1111 = theEventClassifierMap[1111]; // SPECT event, 2->1 recHits of theGammaEnergy keV, LOR < 4mm from source
  G4double b_1112 = theEventClassifierMap[1112]; // SPECT event, 2->1 recHits of theGammaEnergy keV, LOR > 4mm from source
  
  G4cout << " --------------- Random Coincidences and Scattered -------------" << G4endl;
  G4cout << "       ALL COINC. AND SCCATTERED       : " << nCOINC_SCATT << "   " << nCOINC_SCATT/nevt*100 << " % " << G4endl; 
  ...
  G4cout << "( 1111) SPECT 2->1 recHit close to vtx : " << b_1111 << "   " << b_1111/(G4double(nevt))*100 << " % " << G4endl;
  G4cout << "( 1112) SPECT 2->1 recHit far from vtx : " << b_1112 << "   " << b_1112/(G4double(nevt))*100 << " % " << G4endl;
  G4cout << " ---------------------------------------------" << G4endl;

  // recHits with hits from different original gammas
  G4double b_10001 = theEventClassifierMap[10001]; // SPECT event, 1 recHit of theGammaEnergy keV, LOR < 4mm from source
  G4double b_10002 = theEventClassifierMap[10002]; // SPECT event, 1 recHit of theGammaEnergy keV, LOR > 4mm from source
  
  G4double b_10011 = theEventClassifierMap[10011]; // SPECT event, 2->1 recHits of theGammaEnergy keV, LOR < 4mm from source
  G4double b_10012 = theEventClassifierMap[10012]; // SPECT event, 2->1 recHits of theGammaEnergy keV, LOR > 4mm from source

  G4cout << " --------------- Wrong collimator assignment -------------" << G4endl;
  G4cout << "       ALL WRONG COLLIMATOR            : " << nBADCOLL << "   " << nBADCOLL/nevt*100 << " % " << G4endl; 
  ...
  G4cout << "(10011) SPECT 2->1 recHit close to vtx : " << b_10011 << "   " << b_10011/(G4double(nevt))*100 << " % " << G4endl;
  G4cout << "(10012) SPECT 2->1 recHit far from vtx : " << b_10012 << "   " << b_10012/(G4double(nevt))*100 << " % " << G4endl;

  G4double b_20001 = theEventClassifierMap[20001]; // SPECT event, 1 recHit of theGammaEnergy keV, LOR < 4mm from source
  G4double b_20002 = theEventClassifierMap[20002]; // SPECT event, 1 recHit of theGammaEnergy keV, LOR > 4mm from source
  
  G4double b_20011 = theEventClassifierMap[20011]; // SPECT event, 2->1 recHits of theGammaEnergy keV, LOR < 4mm from source
  G4double b_20012 = theEventClassifierMap[20012]; // SPECT event, 2->1 recHits of theGammaEnergy keV, LOR > 4mm from source

  G4cout << " --------------- NO collimator traversed -------------" << G4endl;
  G4cout << "       ALL NO COLLIMATOR               : " << nNOCOLL << "   " << nBADCOLL/nevt*100 << " % " << G4endl; 
  ...
  G4cout << "(20011) SPECT 2->1 recHit close to vtx : " << b_20011 << "   " << b_20011/(G4double(nevt))*100 << " % " << G4endl;
  G4cout << "(20012) SPECT 2->1 recHit far from vtx : " << b_20012 << "   " << b_20012/(G4double(nevt))*100 << " % " << G4endl;
