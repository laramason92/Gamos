...
void DetComptonStudyHistosUA::BookHistos()
{
  ...
  for( unsigned int ii = 0; ii < classifVars.size(); ii++ ){
    ...
    DetVComptClassificationVariable* classif = Reflex::PluginService::Create<DetVComptClassificationVariable*>(varName );
    if( classif == 0 ) {
      G4Exception("DetComptonStudyHistosUA::BookHistos",
    ...
    theCClassifVars.push_back( classif );
    classif->BookHistos(theHistoNumber+(ii+1)*10000);
  }
...
{ 
  ...
  if( procName == "compt" || 
    ...
    if( aStep->GetTrack()->GetVolume()->GetLogicalVolume()->GetSensitiveDetector() != 0 ) {
      ...
      if( DetVerb(debugVerb) ) G4cout << " theInteractions " << aStep->GetTrack()->GetTrackID() << " angle " << ts->GetDeltaAngle()/deg << G4endl;
#endif
