...
G4ThreeVector GmPositionSurfacePos::GeneratePosInSolidSurface( const G4VSolid* solid )
{
  ...
  if( solid->GetEntityType() == "G4Orb" ) {
    ...
    G4bool bFullSphere = (radiusI == 0. && phiStart == 0. && fabs(phiEnd - 2*pi) < angTolerance && costhetaStart == 1. && fabs(costhetaEnd+1.) < angTolerance);
#ifndef GAMOS_NO_VERBOSE
    ...
    G4bool bFullTubs = (radiusInner == 0. && phiStart == 0. && fabs(phiEnd - 2*pi) < angTolerance);
    ...
    G4bool bFullCons = (radiusInnerM == 0. && radiusInnerP && phiStart == 0. && fabs(phiEnd - 2*pi) < angTolerance );
