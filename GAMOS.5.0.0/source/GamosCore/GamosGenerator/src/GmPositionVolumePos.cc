...
G4ThreeVector GmPositionVolumePos::GeneratePosInSolid( const G4VSolid* solid )
{
  ...
  if( solid->GetEntityType() == "G4Orb" ) {
    ...
    if( phiEnd < 0. || phiEnd > 2.*pi ) phiEnd = fmod(phiEnd+2.*pi,2*pi);
    G4double thetaS = sphe->GetStartThetaAngle();
    G4double thetaE = sphe->GetStartThetaAngle() + sphe->GetDeltaThetaAngle();
    if( thetaE < 0. || thetaE > pi ) thetaE = fmod(thetaE+pi,pi);
    //----- most cases it will be a full sphere, and not checking will spare some time
    G4bool bFullSphere = (radI2 == 0. && phiStart == 0. && fabs(phiEnd-2*pi) < angTolerance && thetaS == 0. && fabs(thetaE - 2*pi) < angTolerance);
	...
	G4double phi = fmod(pos.phi()+2.*pi,2.*pi); 
	G4double theta = fmod(pos.theta()+pi,pi);
	if( pos.mag2() < radI2 ||
    ...
    G4bool bFullTubs = (radI2 == 0. && phiStart == 0. && fabs(phiEnd - 2.*pi) < angTolerance );
#ifndef GAMOS_NO_VERBOSE
    if( !bFullTubs ) if( GenerVerb(debugVerb) ) G4cout  << "GmPositionVolumePos::GeneratePosInSolid solid is tube section but not full " << solid->GetName() << " phiStart " << phiStart/deg << " phiEnd " << phiEnd/deg << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
	  if( GenerVerb(debugVerb) ) G4cout  << "GmPositionVolumePos::GeneratePosInSolid small radius " << pos.mag2() << " <? " << radI2 << " phi out " << phiStart/deg << " <? " << phi/deg << " <? " << phiEnd/deg << G4endl;
#endif
	  continue;
	}
      }
    ...
    //----- most cases it will be a full tube section, and not checking will spare some time
    G4bool bFullCons = (radIM == 0. && radIP == 0. && phiStart == 0. && fabs(phiEnd - 2*pi) < angTolerance);
#ifndef GAMOS_NO_VERBOSE
	...
	if( pos.mag2() < radI2 ||
      ...
      x += radTor;
      double Rp = 2.*pi*CLHEP::RandFlat::shoot();
      z = x * sin(Rp);
      x *= cos(Rp);
