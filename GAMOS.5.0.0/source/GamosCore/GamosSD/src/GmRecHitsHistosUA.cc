...
void GmRecHitsHistosUA::EndOfEventAction(const G4Event* )
{
  ...
  for( item = rechitsMap.begin(); item != rechitsMap.end(); item++ ){
    ...
    for( ite = rhits.begin(); ite != rhits.end(); ite++ ){
      ...
      theAnaMgr->GetHisto1(202000+nh*100+2)->Fill( float(rhit->GetEnergy()/keV));
      G4ThreeVector depowidth = GetEDepoWidth( rhit );
      theAnaMgr->GetHisto1(202000+nh*100+3)->Fill( depowidth.mag()/mm );
      theAnaMgr->GetHisto1(202000+nh*100+4)->Fill( depowidth.z()/mm );
      theAnaMgr->GetHisto1(202000+nh*100+5)->Fill( depowidth.phi()/deg );
	...
	if( hit->GetTimeMax() > timemax ) timemax = hit->GetTimeMax();
      ...
      theAnaMgr->GetHisto1(202000+nh*100+7)->Fill( (timemax-timemin)/ns );
	...
	if( dhit > disthits ) disthits = dhit;
      ...
      if( disthits != -1. ) theAnaMgr->GetHisto1(202000+nh*100+8)->Fill( disthits/mm ); 
    }
  }
