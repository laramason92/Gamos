...
void GmHistosGammaAtSD::PostUserTrackingAction(const G4Track* )
{
  ...
  if( theInteractions.GetSteps().size() != 0 ) {
    theEnteringEnergies.push_back( theInteractions.GetStep(0)->GetKineticEnergyPre()/keV );
  }
...
void GmHistosGammaAtSD::FillHistos()
{
  ...
  for( G4int ii = 0; ii < npoints; ii++ ){
    ...
    G4double dener = trajsp->GetDeltaKineticEnergy()/keV ;
    energyLost += dener;
    ...
    if( trajsp->GetProcessName() == "phot" ){ 
      ...
      if( ii != 0 ) {
	G4double posdiff = (trajsp->GetPositionPost() - posFirst).mag()/mm;
	...
	dir /= dir.mag();
	G4double dirdiff = acos( dirFirst * dir ) / mrad;
	theAnaMgr->GetHisto1(103000+icl*100+14)->Fill( dirdiff  );
	theAnaMgr->GetHisto1(103000+14)->Fill( dirdiff  );

	G4double kinEdiff = (kinEFirst - trajsp->GetKineticEnergyPre() )/keV;
	theAnaMgr->GetHisto1(103000+icl*100+15)->Fill( kinEdiff  );
	theAnaMgr->GetHisto1(103000+15)->Fill( kinEdiff  );
#ifndef GAMOS_NO_VERBOSE
	if( SDVerb(testVerb) ) G4cout << "PE: posdiff " << posdiff << " pos " << trajsp->GetPositionPost()/mm << " posFirst " << posFirst << G4endl 
	       << "PE: dirdiff " << dirdiff << " dir " << dir << " dirFirst " << dirFirst << G4endl
	       << "PE: kinEdiff " << kinEdiff << " kinE " << trajsp->GetKineticEnergyPre()/keV << " kinEFirst " << kinEFirst/keV << G4endl; 
#endif
	...
	G4double ang = trajsp->GetDeltaAngle()/mrad;
    ...
    if( ii == 0 ) {
      G4double kinE = trajsp->GetKineticEnergyPre()/keV;
...
void GmHistosGammaAtSD::EndOfRunAction( const G4Run* )
{
  ...
  G4cout << " $$GSD: No PE 0 COMP    : " << c0pecomp << " : " << 100.* (c0pecomp)/(nevents-cPE) << " % " << G4endl;
  G4cout << " $$GSD: No PE 1 COMP    : " << c1COMP-cPE1COMP << " : " << 100.* (c1COMP-cPE1COMP)/(nevents-cPE)  << " % " << G4endl;
  G4cout << " $$GSD: No PE 2 COMP    : " << c2COMP-cPE2COMP << " : " << 100.* (c2COMP-cPE2COMP)/(nevents-cPE)  << " % " << G4endl;
  G4cout << " $$GSD: No PE >2 COMP   : " << cnCOMP-cPEnCOMP << " : " << 100.* (cnCOMP-cPEnCOMP)/(nevents-cPE) << " % " << G4endl;
