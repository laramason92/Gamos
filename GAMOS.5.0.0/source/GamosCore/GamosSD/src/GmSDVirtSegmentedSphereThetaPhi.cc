...
GmSDVirtSegmentedSphereThetaPhi::GmSDVirtSegmentedSphereThetaPhi(G4String sdtype ): GmSDVirtSegmentedSphere( sdtype )
{
  ...
  theRResolution =  parmgr->GetNumericValue("SD:VirtSegmentedSphereThetaPhi:Rresolution:"+sdtype, 0.*ns); // save it here for efficiency
...
void GmSDVirtSegmentedSphereThetaPhi::DefineThetaPhiSegmentation(const G4String& sdtype)
{
  ...
  GmParameterMgr* parmgr = GmParameterMgr::GetInstance();
  G4double cellThetaPhi = 10.*mm;
  cellThetaPhi = parmgr->GetNumericValue("SD:cellThetaPhi:"+sdtype, 10.*mm);
