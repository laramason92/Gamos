...
//----------------------------------------------------------------------
GmHitList::GmHitList( G4String& sdtyp ): theSDType( sdtyp )
{
  GmParameterMgr* parmgr = GmParameterMgr::GetInstance();
  G4String sdtype = GetSDType();
  sdtype = sdtype.substr(1,sdtyp.size()-2);
  
  G4String deadtimeType = parmgr->GetStringValue("SD:DeadTimeType:"+sdtype,"byBlock");

  if( deadtimeType == "byCrystal" ){
    theDeadTimeDetUnitList = new GmDeadTimeDUListByCrystal(sdtyp);
  }else if( deadtimeType == "byBlock" ){
    theDeadTimeDetUnitList = new GmDeadTimeDUListByBlock(sdtyp);
  } else {
  ...
  theMeasuringTime = parmgr->GetNumericValue("SD:MeasuringTime:"+sdtype, 0.*ns); // save it here for efficiency
#ifndef GAMOS_NO_VERBOSE
  if( SDVerb(infoVerb) ) G4cout << "GmHitList sdtype " << sdtype << " theMeasuringTime " << theMeasuringTime << G4endl;
#endif

  theDeadTime = parmgr->GetNumericValue("SD:DeadTime:"+sdtype, 0.*ns); // save it here for efficiency
#ifndef GAMOS_NO_VERBOSE
...
#ifndef GAMOS_NO_VERBOSE
  if( SDVerb(debugVerb) ) G4cout << "GmHitList::BuildHitsCompatibleInTimeTriggerGlobal  looking between times " << lowestTime << " - " << currentTime << G4endl;
#endif
  for( ite = begin(); ite != end(); ite++ ){
    G4bool bOK = true;
    G4double hitTime = (*ite)->GetTime();
    ...
    if( bOK == true) {
      theHitsCompatibleInTime.push_back( *ite );
#ifndef GAMOS_NO_VERBOSE
      if( SDVerb(infoVerb) ) G4cout << "GmHitList::BuildHitsCompatibleInTime  hit OK. "; 
#endif
    } else {
#ifndef GAMOS_NO_VERBOSE
      if( SDVerb(infoVerb) ) G4cout << "GmHitList::BuildHitsCompatibleInTime  hit NOT OK. ";	  
#endif
    }
#ifndef GAMOS_NO_VERBOSE
    if( SDVerb(infoVerb) )
...
#ifndef GAMOS_NO_VERBOSE
  //  if( SDVerb(debugVerb) ) G4cout << "GmHitList::BuildHitsCompatibleInTimeTrigger  currentTime= " << currentTime << " theCurrentIntervalStartTime= " << theCurrentIntervalStartTime << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
    if( SDVerb(debugVerb) ) 
      G4cout << "GmHitList::BuildHitsCompatibleInTimeTrigger  currentTime= " << currentTime
	     << "  TrigID " << trigID  
