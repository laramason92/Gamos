  ...
  for( itehm = hitlists.begin(); itehm != hitlists.end(); itehm++ ){
    ...
    for( iteh = hitVector->begin(); iteh != hitVector->end(); iteh++ ){
      ...
      */
      ...
      theAnaMgr->GetHisto1(201000+nh*100+2)->Fill( float(hit->GetEnergy()/keV));
      G4ThreeVector depowidth = GetEDepoWidth( hit );
      theAnaMgr->GetHisto1(201000+nh*100+3)->Fill( depowidth.mag()/mm );
      theAnaMgr->GetHisto1(201000+nh*100+4)->Fill( depowidth.z()/mm );
      theAnaMgr->GetHisto1(201000+nh*100+5)->Fill( depowidth.phi()/deg );
      theAnaMgr->GetHisto1(201000+nh*100+6)->Fill( float(hit->GetEDepos().size()) );
      theAnaMgr->GetHisto1(201000+nh*100+7)->Fill( float(hit->GetTimeMax()-hit->GetTimeMin())/ns );
      ...
      theAnaMgr->GetHisto1(201000+nh*100+15)->Fill( pos.phi()/deg );
      theAnaMgr->GetHisto1(201000+nh*100+16)->Fill( pos.theta()/deg );
	...
	// if( SDVerb(debugVerb) ) G4cout << ii << " " << ii2 << " disthits " << disthits << " dhit " << dhit << " pos1 " << hit->GetPosition() << " pos2 " << hit2->GetPosition() << " diff " << hit->GetPosition()-hit2->GetPosition() << G4endl;
      ...
      if( disthits != -1. ) theAnaMgr->GetHisto1(201000+nh*100+8)->Fill( disthits/mm );
    }
