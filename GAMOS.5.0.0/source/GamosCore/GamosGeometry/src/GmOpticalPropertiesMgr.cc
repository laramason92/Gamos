...
void GmOpticalPropertiesMgr::CreateOpticalSurface( const std::vector<G4String> wl )
{
  if( theOpticalSurfaces.find(wl[1]) == theOpticalSurfaces.end() ) {
    ...
    if( wl.size() > 4 ){
      ...
      } else if( wl[4] == "firsov"){type = firsov;
      } else if( wl[4] == "x_ray"){type = x_ray;
      } else {
	G4Exception(" GmOpticalPropertiesMgr::CreateOpticalSurface",
		    " Wrong type",
		    FatalErrorInArgument,
		    G4String("Type is: " + wl[4] + ", while it can only be dielectric_metal, dielectric_dielectric, dielectric_LUT, firsov or x_ray"));
      }
    }
    if( wl.size() > 5 ) {
