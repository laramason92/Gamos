...
GmAnalysisMgr::GmAnalysisMgr(const G4String& fileName)
{
...
#ifndef GAMOS_NO_VERBOSE
  if( BaseVerb(infoVerb) ) G4cout << " AnalysisMgr fileName with prefix " << fileName  << G4endl;
#endif

  G4String suffix = GmParameterMgr::GetInstance()->GetStringValue("GmAnalysisMgr:FileNameSuffix","");
  theFileName = theFileName+suffix;
#ifndef GAMOS_NO_VERBOSE
  if( BaseVerb(infoVerb) ) G4cout << " AnalysisMgr fileName with suffix " << fileName  << G4endl;
#endif
  ...
  bNormalizeToNEvents = G4bool(GmParameterMgr::GetInstance()->GetNumericValue(fileName+":NormalizeToNEvents",1));
