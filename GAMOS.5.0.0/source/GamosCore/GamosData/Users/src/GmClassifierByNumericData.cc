  ...
  if( params.size() != 4 ) {
    G4Exception(G4String(theName+"::SetParameters").c_str(),
		"Error in number of parameters",
  		JustWarning,
		G4String("There should be four arguments: DATA_NAME MIN MAX STEP (IS_EXPONENTIAL_STEP), and there are "+GmGenUtils::itoa(params.size())).c_str());
  }
...
#ifndef GAMOS_NO_VERBOSE
  if( DataVerb(debugVerb) ) G4cout << theName << index << " " << val << G4endl;
#endif
