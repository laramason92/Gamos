...
G4int GmClassifierByStringData::GetIndexFromValue(const G4String val )
{
  ...
  if( BaseVerb(debugVerb) ) G4cout << " GmClassifierByStringData::GetIndexFromValue " << theName << " : " << index << " key " << val << " index " << index << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
  if( BaseVerb(debugVerb) ) G4cout << " GmClassifierByStringData::GetIndexName " << theName << " : " << name << " index " << index << G4endl;
#endif
  return name;
}
