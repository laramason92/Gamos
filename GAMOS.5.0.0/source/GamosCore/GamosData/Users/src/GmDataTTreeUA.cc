...
GmDataTTreeUA::GmDataTTreeUA()
{
  ...
  this->theTreeFileName = paramMgr->GetStringValue(theName+"TreeFileName","GmDataTTree");
  ...
  theTTreeStartingIndex = GmParameterMgr::GetInstance()->GetNumericValue(theName+":TreeStartingIndex",1000);
...
void GmDataTTreeUA::SaveTree()
{
  if(this->theTree) {
    ...
    this->theTree->Write("",TObject::kOverwrite);
...
{
  bUseAtInitial = G4bool(GmParameterMgr::GetInstance()->GetNumericValue(theName+":UseAtInitial",0));

  //Reading list for Event Data
  theEventDataList = GmParameterMgr::GetInstance()->GetVStringValue("GmDataTTreeUA:EventDataList",theEventDataList);
  BuildDataFromLoopOnDataList(theEventData,theEventDataList);
#ifndef GAMOS_NO_VERBOSE
  if( DataVerb(infoVerb) ) G4cout << " param " << theName+":EventDataList " << theEventDataList.size() << G4endl;
  if( DataVerb(infoVerb) ) G4cout << " param " << theName+":EventData " << theEventData.size() << G4endl;
#endif

  //Reading list for Step Data
  theStepDataList = GmParameterMgr::GetInstance()->GetVStringValue(theName+":StepDataList",theStepDataList);
  BuildDataFromLoopOnDataList(theStepData,theStepDataList);
#ifndef GAMOS_NO_VERBOSE
  if( DataVerb(infoVerb) ) G4cout << " param " << theName+":StepDataList " << theStepDataList.size() << G4endl;
  if( DataVerb(infoVerb) ) G4cout << " param " << theName+":StepData " << theStepData.size() << G4endl;
#endif

  //Reading list for Track Data
  theTrackDataList = GmParameterMgr::GetInstance()->GetVStringValue(theName+":TrackDataList",theTrackDataList);
  BuildDataFromLoopOnDataList(theTrackData,theTrackDataList);
#ifndef GAMOS_NO_VERBOSE
  if( DataVerb(infoVerb) ) G4cout << " param " << theName+":TrackDataList " << theTrackDataList.size() << G4endl;
  if( DataVerb(infoVerb) ) G4cout << " param " << theName+":TrackData " << theTrackData.size() << G4endl;
#endif

  //Reading list for SecondaryTrack Data
  theSecondaryTrackDataList = GmParameterMgr::GetInstance()->GetVStringValue(theName+":SecondaryTrackDataList",theSecondaryTrackDataList);
  BuildDataFromLoopOnDataList(theSecondaryTrackData,theSecondaryTrackDataList);
#ifndef GAMOS_NO_VERBOSE
  if( DataVerb(infoVerb) ) G4cout << " param " << theName+":SecondaryTrackDataList" << theSecondaryTrackDataList.size() << G4endl;
  if( DataVerb(infoVerb) ) G4cout << " param " << theName+":SecondaryTrackData" << theSecondaryTrackData.size() << G4endl;
#endif
