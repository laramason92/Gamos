...
{
  return aStep->GetPreStepPoint()->GetMaterial()->GetElectronDensity()/g*cm3;
}

//----------------------------------------------------------------
G4double GmDataInitialElectronDensity::GetValueFromTrack( const G4Track* aTrack, G4int )
{
  return aTrack->GetLogicalVolumeAtVertex()->GetMaterial()->GetElectronDensity()/g*cm3;
}

//----------------------------------------------------------------
G4double GmDataInitialElectronDensity::GetValueFromSecoTrack( const G4Track* aTrack1, const G4Track* , G4int )
{
  return aTrack1->GetMaterial()->GetElectronDensity()/g*cm3;
}
...
G4double GmDataInitialElectronDensity::GetValueFromEvent( const G4Event* anEvent, G4int )
{
  ...
  G4double density = touch->GetVolume()->GetLogicalVolume()->GetMaterial()->GetElectronDensity()/g*cm3;
