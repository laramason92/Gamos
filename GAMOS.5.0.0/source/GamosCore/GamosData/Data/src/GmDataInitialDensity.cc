...
{
  return aStep->GetPreStepPoint()->GetMaterial()->GetDensity()/g*cm3;
}

//----------------------------------------------------------------
G4double GmDataInitialDensity::GetValueFromTrack( const G4Track* aTrack, G4int )
{
  return aTrack->GetLogicalVolumeAtVertex()->GetMaterial()->GetDensity()/g*cm3;
}

//----------------------------------------------------------------
G4double GmDataInitialDensity::GetValueFromSecoTrack( const G4Track* aTrack1, const G4Track* , G4int )
{
  return aTrack1->GetMaterial()->GetDensity()/g*cm3;
}
...
G4double GmDataInitialDensity::GetValueFromEvent( const G4Event* anEvent, G4int )
{
  ...
  G4double density = touch->GetVolume()->GetLogicalVolume()->GetMaterial()->GetDensity()/g*cm3;
