...
G4double GmDataRandomPosTheta::GetValueFromStep( const G4Step* aStep, G4int )
{
  ...
  return (aStep->GetPreStepPoint()->GetPosition() + rand*(aStep->GetPostStepPoint()->GetPosition()-aStep->GetPreStepPoint()->GetPosition())).theta()/deg;
}
...
{
  G4double rand = CLHEP::RandFlat::shoot();
  return (aTrack->GetVertexPosition()+rand*(aTrack->GetPosition()-aTrack->GetVertexPosition())).theta()/deg;
}
