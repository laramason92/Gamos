...
{
  return (aStep->GetPostStepPoint()->GetMomentumDirection()-aStep->GetPreStepPoint()->GetMomentumDirection()).phi()/deg;
}

//----------------------------------------------------------------
G4double GmDataDirChangePhi::GetValueFromTrack( const G4Track* aTrack, G4int )
{
  return (aTrack->GetMomentumDirection()-aTrack->GetVertexMomentumDirection()).phi()/deg;
}
