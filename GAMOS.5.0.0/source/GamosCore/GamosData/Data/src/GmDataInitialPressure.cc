...
{
  return aStep->GetPreStepPoint()->GetMaterial()->GetPressure()/atmosphere;
}

//----------------------------------------------------------------
G4double GmDataInitialPressure::GetValueFromTrack( const G4Track* aTrack, G4int )
{
  return aTrack->GetLogicalVolumeAtVertex()->GetMaterial()->GetPressure()/atmosphere;
}

//----------------------------------------------------------------
G4double GmDataInitialPressure::GetValueFromSecoTrack( const G4Track* aTrack1, const G4Track* , G4int )
{
  return aTrack1->GetMaterial()->GetPressure()/atmosphere;
}
...
G4double GmDataInitialPressure::GetValueFromEvent( const G4Event* anEvent, G4int )
{
  ...
  G4double density = touch->GetVolume()->GetLogicalVolume()->GetMaterial()->GetPressure()/atmosphere;
