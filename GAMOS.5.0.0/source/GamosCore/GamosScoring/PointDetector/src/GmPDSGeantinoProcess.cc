...
GmPDSGeantinoProcess::GmPDSGeantinoProcess(const G4String& name, G4SteppingManager* fpSM, G4ParticleDefinition* origParticle ): GmPDSVProcess( name, fpSM )
{
  ...
  theExclusionRadius = GetPDSNumericParameter("ExclusionRadius",partName,1.*mm);
...
G4VParticleChange* GmPDSGeantinoProcess::PostStepDoIt( const G4Track& aTrack, const G4Step& aStep)
{
  ...
  if( DR != -1 ) { 
    ...
    if( theCurrentHelper->bControlHistos ) {
      ...
      theCurrentHelper->theAnaMgr->GetHisto1(nhALL+91)->Fill(dist*cm);
      theCurrentHelper->theAnaMgr->GetHisto2(nhALL+92)->Fill(dist*cm,log10(wei));
    }
    ...
    if( theCurrentHelper->bControlHistos ) {
      theCurrentHelper->theAnaMgr->GetHisto2(nhALL+93)->Fill(dist*cm,log10(wei) * (*ite).second.Hstar);
    }
...
void GmPDSGeantinoProcess::ChangeGeantinoWeight(const G4Step* aStep)
{
  ...
  GmTrackInfo* gmtrki = dynamic_cast<GmTrackInfo*>( trki );
  G4Point3D point = gmtrki->GetThreeVectorValue("Point"); // geantino has in TrackInformation the point it is aimed to
  G4double dist = (point-vtx).mag()/cm;
  if( dist < theExclusionRadius/cm ){
    G4double eRcm = theExclusionRadius/cm;
    G4double wei = GetWeight(aTracknc);
#ifndef GAMOS_NO_VERBOSE
    if( ScoringVerb(infoVerb) ) G4cout << "GmPDSGeantinoProcess::ChangeGeantinoWeight  particle in exclusion sphere " << dist*cm << " < " << theExclusionRadius*cm << " WEIGHT0 = " << wei << G4endl;
#endif
    //--- Take out the 1/2PIR^2 radius
    wei *= (2.*(acos(0.)*2)*pow( dist,2 ) );
#ifndef GAMOS_NO_VERBOSE
    if( ScoringVerb(infoVerb) ) G4cout << " GmPDSGeantinoProcess::ChangeGeantinoWeight  particle in exclusion sphere " << dist*cm << " WEIGHT1 = " << wei << G4endl;
#endif
    
    G4double mfpcm = (1./XS)/cm;
    //    wei *= (2/pow(XS,3.)-exp(-theExclusionRadius*XS)*(2.*pow(mfpcm,3.)+2*eRcm*mfpcm*mfpcm+eRcm*eRcm*mfpcm))/(pow(eRcm,3.)/mfpcm/3.);
    wei *= (1-exp(-theExclusionRadius*XS))/(2./3.*M_PI*pow(eRcm,3.)/mfpcm);
    SetWeight( aTracknc, wei );
#ifndef GAMOS_NO_VERBOSE
    if( ScoringVerb(warningVerb) ) G4cerr << " GmPDSGeantinoProcess::ChangeGeantinoWeight  particle in exclusion sphere " << dist << " WEIGHT = " << wei << " f " << (2.+2*eRcm+eRcm*eRcm) << " " << exp(-theExclusionRadius*XS) << " " << (2-exp(-theExclusionRadius*XS)*(2.+2*eRcm+eRcm*eRcm)) << " " << pow(eRcm,3.) << " " << XS/cm/3. << " XS " << XS <<  G4endl;
#endif
  } else {
