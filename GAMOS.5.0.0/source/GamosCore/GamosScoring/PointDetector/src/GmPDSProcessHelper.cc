...
//GmPDSProcessHelper::GmPDSProcessHelper(const G4String& name, G4SteppingManager* fpSM ): G4VDiscreteProcess( name )5~, fpSteppingManager(fpSM)
{
  ...
  theExclusionRadius = GetPDSNumericParameter("ExclusionRadius",theOriginalParticleName,1.*mm);
...
{
  G4double ener = aTrack.GetKineticEnergy()/MeV;
  if( ener == 0. ) return;
...
{
  return (theDetectorCentres[detID] - pos).mag()/cm;
