  ...
  : GmVScoringSurfaceSolid()
{
  ...
  theNSurfaceMax = 4;
  if( std::fabs(theTubs->GetDeltaPhiAngle() - 360.*deg) < theAngularTolerance ){
    theNSurfaceMax++;
  }
...
void GmScoringSurfaceTubs::ComputeArea()
{
  ...
  if( theSurfaces.find( "PHI" ) != theSurfaces.end() ) {
    if( std::fabs(theTubs->GetDeltaPhiAngle() - 360.*deg) < G4GeometryTolerance::GetInstance()->GetAngularTolerance() ) {
