...
void GmPSPrinterXYZ::WriteHeader()
{
  ...
  for( unsigned int ii = 0; ii <= nx; ii++ ) {
    vval.push_back( (thePhantomMinusCorner.x() + ii*voxelX) / cm );
  ...
  for( unsigned int ii = 0; ii <= ny; ii++ ) {
    vval.push_back( (thePhantomMinusCorner.y() + ii*voxelY) / cm );
  ...
  for( unsigned int ii = 0; ii <= nz; ii++ ) {
    vval.push_back( (thePhantomMinusCorner.z() + ii*voxelZ) / cm );
...
void GmPSPrinterXYZ::WriteValues( G4THitsMap<G4double>* RunMap, GmVPrimitiveScorer* )
{
  ...
  for( G4int ii = 0; ii < nvoxels; ii++ ){
    ...
    *theFileOut << (thePhantomMinusCorner.x() + (ix*2+1)*voxelHalfX)/cm << " " 
		<< (thePhantomMinusCorner.y() + (iy*2+1)*voxelHalfY)/cm << " " 
		<< (thePhantomMinusCorner.z() + (iz*2+1)*voxelHalfZ)/cm << " ";
