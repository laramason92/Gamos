...
void G4ParticleHPInelasticCompFS::Init (G4double A, G4double Z, G4int M, G4String & dirName, G4String & aFSType, G4ParticleDefinition*)
{
  ...
  G4bool dbool;
  G4ParticleHPDataUsed aFile = theNames.GetName(static_cast<G4int>(A), static_cast<G4int>(Z), M, tString, aFSType, dbool, theDataDirVariable);
...
// prepare neutron
    ...
    G4ReactionProduct theIncidentPart( const_cast<G4ParticleDefinition *>(incidentParticle->GetDefinition()) ); // theProjectile
    theIncidentPart.SetMomentum( incidentParticle->Get4Momentum().vect() );
    theIncidentPart.SetKineticEnergy( eKinetic );
...
#ifdef G4PARTICLEHPDEBUG
    if( getenv("G4ParticleHPDebug"))  G4cout <<this <<" G4ParticleHPInelasticCompFS::CompositeApply A " <<theBaseA <<" Z " <<theBaseZ <<" incident " <<incidentParticle->GetDefinition()->GetParticleName() <<G4endl;
#endif
...
//        targetMass = theFinalStatePhotons[50]->GetTargetMass();
    ...
    G4ThreeVector neuVelo = (1./incidentParticle->GetDefinition()->GetPDGMass())*theIncidentPart.GetMomentum();
    theTarget = aNucleus.GetBiasedThermalNucleus( targetMass, neuVelo, theTrack.GetMaterial()->GetTemperature());
    theTarget.SetDefinition( G4ParticleTable::GetParticleTable()->GetIon( G4int(theBaseZ), G4int(theBaseA) , 0.0 ) ); 
...
// prepare energy in target rest frame
    ...
    boosted.Lorentz(theIncidentPart, theTarget);
...
// set target and neutron in the relevant exit channel
    InitDistributionInitialState(theIncidentPart, theTarget, it);    
    ...
    G4double availableEnergy = theIncidentPart.GetKineticEnergy() + theIncidentPart.GetMass() - aHadron.GetMass() +
...
//    TK Excitation level is not determined
      ...
      //aHadron.SetMomentum(theIncidentPart.GetMomentum()*(1./theIncidentPart.GetTotalMomentum())*
      ...
      aHadron.SetMomentum(theIncidentPart.GetMomentum()*(1./theIncidentPart.GetTotalMomentum())*p );
...
// This needs more design @@@
    ...
    if( theParticles==0 )
    {
      ...
      G4ThreeVector incidentNeutronMomentum = theIncidentPart.GetMomentum();
...
//080717 TK Comment still do NOT include photon's mometum which produce by thePhotons
        theResidual.SetMomentum( theIncidentPart.GetMomentum() + theTarget.GetMomentum() - totalMomentum );
