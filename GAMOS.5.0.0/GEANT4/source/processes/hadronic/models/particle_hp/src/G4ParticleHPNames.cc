...
G4ParticleHPDataUsed G4ParticleHPNames::GetName(G4int A, G4int Z, G4int M, G4String base, G4String rest, G4bool & aFlag, G4String dataDirVariable)
{
...
//        if (std::abs(myZ-Z)>theMaxOffSet||myZ==0||myA==0)
          ...
          { 
             ...
             //{
                ...
                G4cout <<"G4ParticleHPNames: Please make sure variable defining HPDATA points to the" << G4endl;
...
// administration and anouncement for lacking of exact data in NDL 
    if ( Z != result.GetZ() || A != result.GetA() )
    {
       ...
       {
          G4String reac = base;
          G4String dir = getenv(dataDirVariable); 
          reac.erase ( 0 , dir.length() );
          if ( getenv ( "G4PARTICLEHP_SKIP_MISSING_ISOTOPES" ) && !( Z == result.GetZ() && result.IsThisNaturalAbundance() ) )
          {
