...
G4int G4ParticleHPProduct::GetMultiplicity(G4double anEnergy, G4bool bForceAverage )
{
  ...
  { 
     ...
     {
       if( theMultiplicityMethod == G4HPMultiPoisson && !bForceAverage ) {
	 multi = G4Poisson ( mean ); 
       } else { //- if( theMultiplicityMethod == G4HPMultiAverage ) {
	...
	//       if( CLHEP::RandFlat::shoot() > mean-multi ) multi++;
     ...
     if( getenv("G4ParticleHPDebug") ) G4cout << theMass << " multi " << multi << " mean " << mean << G4endl;
#endif
  }
...
G4ReactionProductVector * G4ParticleHPProduct::Sample(G4double anEnergy, G4int multi)
{
  ...
  theDist->SetIncidentParticle(theIncidentPart);
  G4int i;
