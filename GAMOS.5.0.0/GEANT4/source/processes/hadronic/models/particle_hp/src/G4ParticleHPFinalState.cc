...
void G4ParticleHPFinalState::adjust_final_state ( G4LorentzVector init_4p_lab )
{
   ...
   else
   {
	...
	resi_pd = G4ParticleTable::GetParticleTable()->GetIon ( int(baseZNew - sum_Z) , (int)(baseANew - sum_A) , 0.0 );
      }
...
#endif
	   oneMoreSec_pd = G4ParticleTable::GetParticleTable()->GetIon ( int(baseZNew - sum_Z) , (int)(baseANew - sum_A) , 0.0 );
	   if( !oneMoreSec_pd ) {
	     G4cerr << "G4ParticleHPFinalState oneMoreSec_pd Z " << baseZNew << " - " << sum_Z << " A " << baseANew << " - " << sum_A << " projectile " << theProjectile->GetParticleName() << G4endl;
