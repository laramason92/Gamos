...
G4ParticleHPInelasticData::G4ParticleHPInelasticData(G4ParticleDefinition* projectile,const char* dataDirVariable)
  :G4VCrossSectionDataSet(G4String(dataDirVariable)+"InelasticXS")
{
  ...
  G4cout << "@@@ G4ParticleHPInelasticData instantiated for particle " << projectile->GetParticleName() << " data directory variable is " << dataDirVariable << " pointing to " << getenv(dataDirVariable) << G4endl;
   ...
   BuildPhysicsTableHP(projectile, dataDirVariable);
                                                ...
                                                const G4Material* /*mat*/ )
{
   ...
   if ( eKin > GetMaxKinEnergy() 
     ...
     || dp->GetDefinition() != thePar ) return false;                                   
...
//080428
   if ( getenv( "G4PARTICLEHP_NEGLECT_DOPPLER" ) ) 
   {
      G4cout << "Find environment variable of \"G4PARTICLEHP_NEGLECT_DOPPLER\"." << G4endl;
...
// TKDB
  ...
  for( size_t i=0; i<numberOfElements; ++i )
  {
    G4PhysicsVector* physVec = G4ParticleHPData::
      Instance(projectile, dataDirVariable)->MakePhysicsVector((*theElementTable)[i], this);
...
//080428
  ...
  G4ThreeVector neutronVelocity = 1./thePar->GetPDGMass()*theNeutron.GetMomentum();
  G4double neutronVMag = neutronVelocity.mag();

#ifndef G4PARTICLEHP_DOPPLER_LOOP_ONCE
...
/*
  // Checking impact of  G4PARTICLEHP_NEGLECT_DOPPLER
