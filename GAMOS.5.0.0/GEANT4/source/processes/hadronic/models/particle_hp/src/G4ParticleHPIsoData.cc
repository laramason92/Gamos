...
G4bool G4ParticleHPIsoData::Init(G4int A, G4int Z, G4int M, G4double abun, G4String dirName, G4String aFSType, G4String dataDirVariable)
  {
    theChannelData = 0;
    ...
    G4ParticleHPDataUsed aFile = theNames.GetName(A, Z, M, dirName, aFSType, result, dataDirVariable);
...
void G4ParticleHPIsoData::Init(G4int A, G4int Z, G4int M,G4double abun, G4ParticleDefinition* projectile, const char* dataDirVariable ) //fill PhysicsVector for this Isotope
  {
      ...
      if(Z>87) //TK Modifed for ENDF VII.0 
	{
	  ...
	  Init(A, Z, M, abun, dirName, "/CrossSection",dataDirVariable);
	}
      else
	{
	  theChannelData = new G4ParticleHPVector;
