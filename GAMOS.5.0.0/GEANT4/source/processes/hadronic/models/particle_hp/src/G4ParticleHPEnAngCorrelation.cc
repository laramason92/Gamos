...
G4ReactionProductVector * G4ParticleHPEnAngCorrelation::Sample(G4double anEnergy)
{
  ...
  /* G4int nResults = 1;
 const char* ctmp = getenv("G4PARTICLEHP_N_SECOS_LOOP");
 if( ctmp ) {
   nResults = atoi( ctmp );
 }

  for( int ir = 0; ir < nResults; ir++ ) {
    if( ir != nResults-1 )  result->clear();
 */
  if(frameFlag==2 
     || frameFlag==3) // Added for particle HP
  {
    // simplify and double check @
    G4ThreeVector the3IncidentPart = theIncidentPart.GetMomentum(); //theIncidentPart has value in LAB
    G4double nEnergy = theIncidentPart.GetTotalEnergy();
    ...
    aIncidentPart.Lorentz(theIncidentPart, theCMS);
    //TKDB 100413 
    ...
    anEnergy = theIncidentPart.GetKineticEnergy(); //should be same argumment of "anEnergy"
  ...
  do {
	...
	G4int nPart;
	if( massCode >= 3000 ) { 
	  nPart = theProducts[i].GetMultiplicity(anEnergy,1);
	} else {
	...
	if( getenv("G4ParticleHPDebug") ) G4cout << "G4ParticleHPEnAngCorrelation::MULTIPLICITY " << massCode << " sumZ " << sumZ << " sumA " << sumA << " NPART " << nPart << G4endl;
#endif
	...
	if( getenv("G4ParticleHPDebug") ) 
	  G4cerr << " WRONG MULTIPLICITY Z= " << sumZ 
		 << " >= " << theTarget.GetDefinition()->GetAtomicNumber() 
		 << " A= " <<  sumA 
		 << " >= " << theTarget.GetDefinition()->GetAtomicMass()<< G4endl;
#endif
      }
    }
  ...
  for(i=0; i<nProducts; i++)
  {
    ...
    if(it!=0)
    {
      ...
      {
      if(!getenv("G4ParticleHP_NO_LORENTZ_BOOST")) {
	...
	if( getenv("G4ParticleHPDebug") ) 
      ...
      }//getenv("G4ParticleHP_NO_LORENTZ_BOOST")) 
      delete it;
    }
  }   

  /*  G4ReactionProductVector::const_iterator ite;
  G4int nn = 0;
  for( ite = result->begin(); ite != result->end(); ite++, nn++ ) {
    G4cout << ir << " " << nn << "INCIDENT_E= " << anEnergy << " PRODUCT " << (*ite)->GetDefinition()->GetParticleName() << " E= " << (*ite)->GetKineticEnergy() << " ETOT= " << (*ite)->GetTotalEnergy() << " theta " << (*ite)->GetMomentum().theta() << G4endl;
  }
