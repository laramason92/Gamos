...
#include "G4LPhysicsFreeVector.hh"
  ...
  G4ParticleHPData::G4ParticleHPData(G4ParticleDefinition* projectile, const char* dataDirVariable)
  {
     numEle = G4Element::GetNumberOfElements();
     theData = new G4ParticleHPElementData[numEle];
//     G4cout << "G4ParticleHPData::G4ParticleHPData(): numEle="<<numEle<<G4endl;
     for (G4int i=0; i<numEle; i++)
     {
       theData[i].Init((*(G4Element::GetElementTable()))[i], projectile, dataDirVariable);
     }
  }
  ...
  G4ParticleHPData * G4ParticleHPData::Instance(G4ParticleDefinition* projectile, const char* dataDirVariable)
  {
    static G4ParticleHPData theCrossSectionData(projectile,dataDirVariable);
