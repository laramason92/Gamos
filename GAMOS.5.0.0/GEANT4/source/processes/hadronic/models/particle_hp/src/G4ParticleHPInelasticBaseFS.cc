...
void G4ParticleHPInelasticBaseFS::Init (G4double A, G4double Z, G4int M, G4String & dirName, G4String & bit, G4ParticleDefinition* )
{
  ...
  G4String tBase = getenv(theDataDirVariable);
  gammaPath = tBase+gammaPath;
  G4String tString = dirName;
  G4bool dbool;
  G4ParticleHPDataUsed aFile = theNames.GetName(static_cast<G4int>(A), static_cast<G4int>(Z), M,tString, bit, dbool, theDataDirVariable);
...
// prepare neutron
  ...
  G4ReactionProduct theIncidentPart( const_cast<G4ParticleDefinition *>(incidentParticle->GetDefinition()) );
...
// Take N-body phase-space distribution, if no other data present.
  if(!HasFSData()) // adding the residual is trivial here @@@
  {
    ...
    thePhaseSpaceDistribution.SetIncidentParticle(&theIncidentPart);
...
// set target and neutron in the relevant exit channel
  if(theAngularDistribution!=0) 
  {
    ...
    theAngularDistribution->SetIncidentParticle(theIncidentPart);
  }
  else if(theEnergyAngData!=0)
  {
    theEnergyAngData->SetTarget(theTarget);
    theEnergyAngData->SetIncidentParticle(theIncidentPart);
  }
  ...
  else if(theAngularDistribution!= 0)
  {
	...
	if(theEnergyDistribution==0 && nDef == 2)
	{
	  if(i0==0)
	  {
	    ...
	    G4double mn = theProjectile->GetPDGMass();
	  ...
	  aHadron->SetMomentum(bufferedDirection);
          aHadron->Lorentz(*aHadron, -1.*(theTarget+theIncidentPart)); 
	  if(getenv("HTOKEN")) 
	  {
