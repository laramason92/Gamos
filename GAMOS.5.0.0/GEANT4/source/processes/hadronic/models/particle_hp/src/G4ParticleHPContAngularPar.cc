...
#include "G4ParticleTable.hh"
  ...
  void G4ParticleHPContAngularPar::Init(std::ifstream & aDataFile, G4ParticleDefinition* projectile)
  { 
                                    ...
                                    G4int angularRep, G4int /*interpolE*/ )
  {
    ...
    if( getenv("G4ParticleHPDebug") ) 
      G4cout << massCode << " G4ParticleHPContAngularPar::Sample E " << anEnergy << G4endl;
#endif
...
*/
             ...
             if ( remaining_energy >= theAngular[i].GetLabel() )
             {
                ...
                double runi = theInt.GetBinIntegral(theManager.GetScheme(i-1),
                                 theAngular[i-1].GetLabel(), theAngular[i].GetLabel(),
                                 theAngular[i-1].GetValue(0), theAngular[i].GetValue(0));
                running[i] += runi;
                weighted += theInt.GetWeightedBinIntegral(theManager.GetScheme(i-1),
...
#ifdef G4PARTICLEHPDEBUG
		if( getenv("G4ParticleHPDebug") ) 
		  G4cout << massCode << " ContAngularPar  running " << i << " " << runi << " SUM " << running[i] << G4endl;
#endif
...
#ifdef G4PARTICLEHPDEBUG
	if( getenv("G4ParticleHPDebug") ) 
	              G4cout << massCode << " ContAngularPar  running " << i << " " << running[i] << " random " << random << G4endl;
#endif
    ...
    else if(angularRep==2)
    {
      ...
      for(i=1; i<nEnergies; i++)
      {
        ...
        double r1 = theInt.GetBinIntegral(theManager.GetScheme(i-1),
                             theAngular[i-1].GetLabel(), theAngular[i].GetLabel(),
                             theAngular[i-1].GetValue(0), theAngular[i].GetValue(0));
        running[i] += r1;
        double w1 = theInt.GetWeightedBinIntegral(theManager.GetScheme(i-1),
                             theAngular[i-1].GetLabel(), theAngular[i].GetLabel(),
                             theAngular[i-1].GetValue(0), theAngular[i].GetValue(0));
        weighted += w1;
#ifdef G4PARTICLEHPDEBUG
	if( getenv("G4ParticleHPDebug") ) 
		G4cout << massCode << " G4ParticleHPContAngularPar " << i << " E= " << theAngular[i].GetLabel() << " - " << theAngular[i-1].GetLabel() << " *XS " << theAngular[i-1].GetValue(0) << " XS2 " << theAngular[i].GetValue(0) << " = running " << r1 << " wei " << w1 << G4endl;
#endif
      }
      // cash the mean energy in this distribution
      //080409 TKDB
...
#ifdef G4PARTICLEHPDEBUG
	if( getenv("G4ParticleHPDebug") ) 
            G4cout << massCode << " G4ParticleHPContAngularPar fsEnergy " << fsEnergy << " it " << it << " randkal " << randkal <<  " x " << x << " x1 " << x1 << " x2 " << x2 << " y1 " << y1 << " y2 " <<y2 << " scheme " << theManager.GetInverseScheme(it-1)%CSTART_ << G4endl;
#endif
