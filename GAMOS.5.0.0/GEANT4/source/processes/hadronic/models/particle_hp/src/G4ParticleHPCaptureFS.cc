...
// dice the photons
    ...
    { 
       ...
       if ( hasExactMF6 )
       {
          ...
          theMF6FinalState.SetIncidentParticle(theNeutron);
...
#include <sstream> 
  void G4ParticleHPCaptureFS::Init (G4double A, G4double Z, G4int M, G4String & dirName, G4String &, G4ParticleDefinition*)
  {
    ...
    G4String tString = "/FS";
    G4bool dbool;
    G4ParticleHPDataUsed aFile = theNames.GetName(static_cast<G4int>(A), static_cast<G4int>(Z), M, dirName, tString, dbool, theDataDirVariable);
