...
//
//
// $Id: G4ParticleHPInelasticCompFS.hh,v 1.1 2012/10/02 17:20:45 arce Exp $
// GEANT4 tag $Name:  $
//
...
class G4ParticleHPInelasticCompFS : public G4ParticleHPFinalState
{
                                           ...
                                           G4int it)
  {
    if(theAngularDistribution[it]!=0) 
    {
      ...
      theAngularDistribution[it]->SetIncidentParticle(anIncidentPart);
    }
    if(theEnergyAngData[it]!=0)
    {
      theEnergyAngData[it]->SetTarget(aTarget);
      theEnergyAngData[it]->SetIncidentParticle(anIncidentPart);
    }
  }
