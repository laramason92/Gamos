...
Color_t GetColor( int icol )
{
  if( icol >= 4 ) icol++; // skip yellow 

  if( icol >= 9 ) icol++; // skip white

  if( icol >= 17 ) icol++; // skip white
  if( icol >= 18 ) icol++; // skip white

  return Color_t(kBlack+icol);
}
